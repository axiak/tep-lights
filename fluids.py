...
class Fluids (dmxwidget.Widget) :
    def init(self, panel) :
        ...
        self.heights[5][5] = 1.0
        self.heights[5][4] = 0.5
        self.heights[5][6] = 0.5
        self.heights[4][5] = 0.5
        self.heights[6][4] = 0.5
        self.boardwidth = 2.0
        self.boardheight = 2.0
    def f(self, x, y, theta) :
        return math.cos(10*x*x*math.cos(0.1*theta) + 10*y*y*math.sin(0.1*theta)+math.cos(0.011*theta)*x*y)
    def updateFluid(self) :
        dampen=0.009 # higher is more
        c2 = 0.1
        dt = 0.05
        ...
        if random.random() < 0.008 :
            self.heights[random.randrange(0, height)][random.randrange(0, width)] += 2*random.random()-1
    ...
    def draw(self, panel) :
        ...
        for r in range(0, panel.height) :
            for c in range(0, panel.width) :
                dx = (self.heights[r][(c+1)%panel.width]-self.heights[r][c])/h
                dy = (self.heights[(r+1)%panel.height][c]-self.heights[r][c])/h
                intens = (-dx*sunx - dy*suny + sunz)/math.sqrt(1 + dx*dx + dy*dy)
                intens = (1+intens)/2
                panel.lights[r][c].sethue(0.8, intens, max(0.0,4.0*intens-3.0))
        panel.outputAndWait(30)
