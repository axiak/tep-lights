...
def servers(request):
    servers = ServerInfo.objects.filter(expires__gte = datetime.datetime.now()).order_by('server_name')
...
def devices(request, server=None):
    ...
    if server:
        ...
        serverinfo = None
        devices = []
        for server in ServerInfo.objects.filter(expires__gte = datetime.datetime.now()).order_by('server_name'):
